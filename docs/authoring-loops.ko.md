...
> **사전 준비물:** `odin-loop` 플러그인이 설치된 Claude Code(`/odin-loop:odin list`가 동작),
> 작업할 프로젝트 디렉터리, 그리고 기본적인 YAML 지식. 플러그인 설치나 일상적인
> `run`/`step`/`status` 동작 원리는 이 가이드의 범위 밖입니다 — 자세한 내용은
> `docs/features.ko.md`를 참고하세요.
...
- **gate mode(게이트 모드)** — *누가* 판정하고, 루프가 멈추는지 여부:
  ...
  - `ai+human` — 엔진이 check를 판정한 뒤, 진행 전에 **여러분의 승인을 위해
    멈춥니다**.
  - `human` — 오늘날은 **`ai+human`의 별칭**입니다: 엔진이 먼저 check를 판정한 뒤
    여러분을 위해 멈춥니다(AI 판정을 건너뛰고 직접 결정하는 별도 모드가 아닙니다).
    제공되는 루프 중 이걸 쓰는 건 없습니다 — `ai+human`을 쓰세요.
...
description: one-liner   # `/odin-loop:odin list`에 표시됨
...
### (b) `/odin-loop:odin new` 사용 (가이드 인터뷰)
...
내용을 다시 보여 주며, `/odin-loop:odin run <name>`으로 시작하도록 안내합니다.
...
`.odin-loop/loops/tech-docs.yaml`에 있습니다 — 이는 **여러분이 자기 프로젝트에서
직접 작성하는** 예시 루프이지, 플러그인에 번들된 루프가 아닙니다(플러그인은
`spec-harness-tdd` 하나만 제공). 그 `.odin-loop/loops/` 경로는 여러분 프로젝트의
커스텀 루프 디렉터리로, `/odin-loop:odin new`가 파일을 쓰는 곳입니다.
...
`/odin-loop:odin new`와 `/odin-loop:odin run`이 시작 전에 대신 실행해 줍니다. (PyYAML이 필요하며, 설치되어
있지 않으면 exit `3`로 위 체크리스트로 폴백합니다.)

그런 다음 엔진이 루프를 인식하는지 확인하고 시작하세요:

```
/odin-loop:odin list              # 여러분의 루프가 description과 스테이지 수와 함께 표시됨
/odin-loop:odin run <name>        # 첫 스테이지부터 루프 시작
/odin-loop:odin status            # 언제든 현재 실행 상태 확인
```

`/odin-loop:odin run <name>`을 실행하면, 엔진은 실행(run)을 생성하고, 첫 스테이지를 현재
스테이지로 설정한 뒤, 그것을 실행하고 게이트를 평가합니다. 그 첫 게이트가
`ai+human`이면 루프는 멈춰 여러분의 `/odin-loop:odin run` 승인을 기다리고, `ai`이면 자동으로
진행합니다.

**다음으로 (이 가이드의 범위 밖):**

- 과거 실행을 바탕으로 루프를 개선하려면 `/odin-loop:odin refine`과 **muninn** 스킬을
  참고하세요 — 작성과는 별개의 워크플로입니다.
- `run`, `step`, `status`의 전체 동작 원리는 `docs/features.ko.md`와
  `docs/design.ko.md`를 참고하세요.

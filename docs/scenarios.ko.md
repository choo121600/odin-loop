...
/odin-loop:odin run
  ...
  → ⏸ ai+human 게이트: 스펙 검토 후 /odin-loop:odin run 으로 승인
/odin-loop:odin run
  → plan: spec.md → plan.md — 빌드 단위, 파일 타겟, 빌드 순서 (inline)
  → ⏸ ai+human 게이트: 계획 검토 후 /odin-loop:odin run 으로 승인
/odin-loop:odin run
...
## 시나리오 2 — 커스텀 루프 작성 (`/odin-loop:odin new`)
...
/odin-loop:odin new
  ...
  → .odin-loop/loops/<name>.yaml 작성(검증됨) 후 /odin-loop:odin run <name> 제안
```

루프가 그저 데이터이기 때문에, 당신의 커스텀 루프는 기본 루프와 정확히 같은 엔진과
게이트를 통해 실행됩니다.

## 시나리오 3 — 루프 교정 (`/odin-loop:odin refine`)

...
`/odin-loop:odin run`은 당신을 운전석에 둡니다 — 모든 `ai+human` 게이트에서 승인을 위해 멈추죠. 그런데
...
- **Odin-Loop 설치** 및 동작(`/odin-loop:odin run`이 루프를 굴려 줌).
...
엄격하게 유지). 이건 `/odin-loop:odin new`나 수동 복사로 하는 **의식적인 작성 행위**여야 합니다. 스케줄러가
대신 해 주지 않습니다.
...
`claude -p "/odin-loop:odin run daily-issue-plan" --settings <프로파일>`을 실행합니다.
...
cron·설치 상태·동의된 외부 행위에 더해 **런타임 건강 상태** — 마지막 발화 시각+결과(또는
`never`)·최근 실패 수·다음 발화 시각(cron에서 계산) —까지 함께 모든 스케줄을 나열:
...
모든 명령은 `/odin-loop:odin schedule <서브커맨드>`이고, 내부적으로 `scripts/hermod.py`를 호출합니다(직접
호출도 가능 — CI/스크립팅에 유용):
...
- [기능](features.ko.md) — `schedule`을 포함한 모든 `/odin-loop:odin` 명령.
